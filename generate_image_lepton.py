...
# 需要继承 Photon 类
class StyleTransferwithCNN(Photon):
    """
    ...
    # 用这个装饰器表示这个一个对外接口
    @Photon.handler("run")
    def style_transfer(self, content_image_url: str, style_image_url: str) -> PNGResponse:
    ...
    #     return "generated_image.jpg"
        ...
        save_result(generated_image, result_height, result_width, name)
        generated_image = Image.open(name)

        edges = Image.fromarray(generated_image)
